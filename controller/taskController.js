    ...
    async getTopEarner(year) {
        ...
        try {
            ...
            if (result?.data?.transactions?.length) {
                 ...
                 */
                const transForGivenYear = result?.data?.transactions?.filter((trans) => new Date(trans?.timeStamp).getFullYear().toString() === year?.toString());

                (transForGivenYear || []).forEach((trans) => {
                 ...
                 */
                result.data.transactions = transForGivenYear?.filter((trans) => trans?.employee?.id?.toLowerCase() === id.toLowerCase() && trans?.type?.toLowerCase() === 'alpha');
            }
        } catch (error) {
            console.log(error);
